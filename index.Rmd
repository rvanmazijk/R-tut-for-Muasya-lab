---
title: "R tutorial for David Klop"
...
We use this notation if we wish to access a specific position's value...
...
Vectors in R are composed of items of a single type (e.g. numerics, characters, logicals, etc.). Should you want to mix types, you must construct a "list", or else R will "coerce" all the items in the vector to the same type (usually characters)
...
If I wanted to move to a directory completely outside of the default, you can do say `setwd("~/Desktop/MSc-analyses/")` or something, with a complete path.
...
### Side-note if Excel has made a "weird" CSV

Often Excel will export a CSV with decimal-commas (e.g. `1,04`) instead of decimal-points (e.g. `1.04`), and semi-colons (`;`) as the separator in the CSV instead of commas (`,`)---which looks something like this when viewed in a text-editor
...
In this instance, you would import the data as follows

```{r, eval=FALSE}
iris <- read.csv("iris.csv", sep = ";", dec = ",")
```

Alternatively, Excel may have saved every cell as character data (enclosed by quotes `"` and `"`) and used a semi-colon as a separator---which looks something like this when viewed in a text-editor
...
Note that Excel has still used decimal-points (`.`), but not stored the numbers as real numeric data.
...
... and then ammend the necessary columns in R to numeric, instead of character, data

```{r, eval=FALSE}
iris[, 1:4] <- as.numeric(as.character(iris[, 1:4]))
```
